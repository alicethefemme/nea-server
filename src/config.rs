...
impl Config {
    pub fn new(config_file_path: String) -> Config {
        ...
        match fs::read_to_string(&config_file_path) {
            Ok(content) => {
                ...
                for item in keys_vals {
                    ...
                    match key {
                        "METRIC_LOGGING_INTERVAL" => {config.metrics.logging_interval = value.parse::<i8>().unwrap();}
                        &_ => {println!("WARNING: Config has item {:?} with value {:?} that is unassigned.", key, value)}
                    }
                }
                config
            }
...
fn create_config_file(config_file_path: String) {
    ...
    match fs::write(config_file_path, download_config_file()) {
        ...
        Err(_) => {
            panic!("Unable to write config file! Please download this from {:?}", SERVER_CONF_FILE_LINK);
        }
    }
}
